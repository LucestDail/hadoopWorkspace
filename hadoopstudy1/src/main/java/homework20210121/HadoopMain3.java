...
public class HadoopMain3 {
	public static void main(String[] args) throws IOException {
		...
		String output = "outfile/"+filename + "inTime";
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter)
			...
			while (itr.hasMoreElements()) {
				...
				String time = word.toString().split(",")[1];
				String timeValue = time.substring(time.length()-2, time.length());
				...
				nameValue1.set(timeValue + " 승차");
				nameValue2.set(timeValue + " 하차");
