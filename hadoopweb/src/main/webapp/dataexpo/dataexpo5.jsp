...
<body>
<h3>월별 출발 / 도착 (정시/지연/조기), 운항거리 분석</h3>
<form action="${pageContext.request.contextPath}/MonMultiServlet2" method="post" name="f">
	...
	<input type = "radio" name = "kbn" value = "d" <core:if test="${param.kbn=='d'}">checked="checked"</core:if>>지연&nbsp;&nbsp;
	<input type = "radio" name = "kbn" value = "e" <core:if test="${param.kbn=='e'}">checked="checked"</core:if>>조기&nbsp;&nbsp;
...
<core:forEach items="${list}" var="map" varStatus="stat">
				...
				labels : [<core:forEach items="${list[0]}" var="m">"${fn:split(m.key,'-')[1]}월",</core:forEach>]
			},
