...
public class CarrMultiTypeReducer extends Reducer<Text, IntWritable, Text, IntWritable>{
	...
	public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException{
		...
		if (columns[0].trim().equals("delay")) {// 출발 지연
			int sum = 0;// 출발 지연 횟수 합산
			for (IntWritable value : values) {
				sum += value.get();
			}
			result.set(sum);// 출력갑 설정
			mos.write("delay", outkey, result);// 출력 데이터 생성
		} else if(columns[0].equals("dstnc")){
			int sum = 0;
			for (IntWritable value : values) {
				sum += value.get();
			}
			result.set((int)(sum/1000));// 출력갑 설정
			mos.write("dstnce", outkey, result);// 출력 데이터 생성
			
		}else if(columns[0].equals("early")){
			int sum = 0;
			for (IntWritable value : values) {
				sum += value.get();
			}
			result.set((int)(sum/1000));// 출력갑 설정
			mos.write("early", outkey, result);// 출력 데이터 생성
