...
import dataexpo.mapper.CarrArrivalDelayMapper;
import dataexpo.mapper.CarrDepartureDelayMapper;
import dataexpo.reducer.CarrDelayReducer;

@WebServlet("/CarrDelayServlet") 
public class CarrDelayServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public CarrDelayServlet() {
        super();
    }
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String output = request.getSession().getServletContext().getRealPath("/") + "output/cardelay/" + year + "_" + kbn;
		Configuration conf = new Configuration();
		try {
			System.out.println("into try");
			Job job = new Job(conf,"CarrDelayServlet");
...
//		request.setAttribute("file", year);
		...
		RequestDispatcher v = request.getRequestDispatcher("/dataexpo/dataexpo2" + view + ".jsp");
